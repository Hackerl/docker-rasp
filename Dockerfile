...
RUN yum install -y unzip openssl-devel bzip2-devel vim-common libtool automake bison gettext && yum clean all
...
RUN curl -fsSL https://nodejs.org/dist/v14.18.0/node-v14.18.0.tar.gz | tar -xz -C /tmp && cd /tmp/node-v14.18.0 && ./configure --openssl-no-asm && LDFLAGS=-lrt make -j$(nproc) && make install && rm -rf /tmp/node-v14.18.0
